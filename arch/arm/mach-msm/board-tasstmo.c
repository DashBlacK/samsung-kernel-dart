...
#ifdef CONFIG_ARCH_MSM7X27
#if defined(CONFIG_MACH_TASSTMO) || defined(CONFIG_MACH_TASSMCT) || defined(CONFIG_MACH_TASSVTR)
...
{
	/* Active High */
	return(gpio_get_value(GPIO_SEND_END) || gpio_get_value(GPIO_SEND_END_USA));
}
...
static struct sec_jack_platform_data sec_jack_data = {
	...
	.send_int_usa	= MSM_GPIO_TO_INT(GPIO_SEND_END_USA),
};
...
static struct snd_endpoint snd_endpoints_list[] = {
	...
	SND(BT_NSEC_OFF, 32),	
	SND(HAC, 33),	
	SND(GANLITE_HANDSET, 34),	
	SND(GANLITE_HEADSET, 35),	
	SND(GANLITE_SPEAKER, 36),	
	SND(NO_MIC_HEADSET_MP3, 37),	
	SND(CURRENT, 39),
};
#undef SND
...
static void keypad_gpio_control(struct platform_device *pdev)
{
	...
	for(i=0; i<minfo->ninputs; i++)	// KBR
		{
//		config = GPIO_CFG(minfo->input_gpios[i], 0, GPIO_INPUT, GPIO_PULL_DOWN, GPIO_2MA);
	...
	// for COOPER
	config = GPIO_CFG(83, 0, GPIO_INPUT, GPIO_PULL_UP, GPIO_2MA);
	ret = gpio_tlmm_config(config, GPIO_ENABLE);
	if (ret) 
		printk(KERN_ERR "%s: gpio_tlmm_config(%#x)=%d\n", __func__, minfo->output_gpios[i], ret);
...
//	GPIO_CFG(1,  0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA), /* CAM_3M_STBY */
//	GPIO_CFG(2,  1, GPIO_INPUT, GPIO_PULL_DOWN, GPIO_2MA), /* CAM_CIF_STBY */
	GPIO_CFG(3,  0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA), /* CAM_CIF_nRST */
...
#if 1//PCAM
	GPIO_CFG(15, 0, GPIO_INPUT, GPIO_PULL_DOWN, GPIO_16MA), /* MCLK */
#else//ORG
	GPIO_CFG(15, 1, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_16MA), /* MCLK */
#endif//PCAM
//        GPIO_CFG(60, 1, GPIO_OUTPUT, GPIO_PULL_UP, GPIO_16MA), /* SCL */
//        GPIO_CFG(61, 1, GPIO_OUTPUT, GPIO_PULL_UP, GPIO_16MA), /* SDA */
...
void wlan_setup_power(int on, int detect)
{
	...
	if (on) {
		...
		gpio_tlmm_config(GPIO_CFG(gpio_wlan_reset_n,0,GPIO_OUTPUT,GPIO_NO_PULL, GPIO_16MA),GPIO_ENABLE); // nextfree.kim 2010-08-24 Wifi ¨ùoA¢´(AOA¨úE¡© ¡íc¢¯©ª ¨ùoA¢´ ¡ícC¡¿ ¢¬OAo)
...
#if defined(CONFIG_MACH_TASSTMO) || defined(CONFIG_MACH_TASSMCT) || defined(CONFIG_MACH_TASSVTR)/* Atheros */
		if (gpio_get_value(BT_PWR) == 0) {
			mdelay(30);
			wlan_setup_clock(0);		
		}
...
#ifndef ATH_POLLING
	mdelay(500); /* NAGSM_ANDROID_WIFI_CJ Prevent not to occured mmc1 timeout */
	if (detect) {
		/* Detect card */
...
#if defined(CONFIG_MACH_TASSTMO) || defined(CONFIG_MACH_TASSMCT) || defined(CONFIG_MACH_TASSVTR)
MACHINE_START(MSM7X27_SURF, "SGH-T499 Board")
#ifdef CONFIG_MSM_DEBUG_UART
	.phys_io        = MSM_DEBUG_UART_PHYS,
	.io_pg_offst    = ((MSM_DEBUG_UART_BASE) >> 18) & 0xfffc,
#endif

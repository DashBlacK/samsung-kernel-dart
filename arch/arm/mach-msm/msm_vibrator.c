...
static int msm_vibrator_power(int on)
{
	...
	if(on) {
		...
		ret = vreg_set_level(vreg_msm_vibrator, OUT3300mV);
#else
...
#if defined(CONFIG_MACH_EUROPA) || defined(CONFIG_MACH_CALLISTO) || defined (CONFIG_MACH_GT2)|| defined(CONFIG_MACH_FLIPBOOK)
#if defined(CONFIG_MACH_CALLISTO)
	if ( board_hw_revision < 1 ) 
	{
#endif

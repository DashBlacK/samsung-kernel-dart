...
// enable this feature to display wakelock
//#define FEATURE_SAMSUNG_DISPLAY_WAKELOCK
 ...
 */
#ifdef FEATURE_SAMSUNG_DISPLAY_WAKELOCK
extern void print_active_locks(int type);
#endif
...
#ifdef FEATURE_SAMSUNG_DISPLAY_WAKELOCK
    active_lock_count++;
    if (active_lock_count==50)
    {
        active_lock_count=0;
        print_active_locks(0);
    }
#endif
...
static int msm_pm_enter(suspend_state_t state)
{
	...
	mdp_pipe_ctrl(MDP_MASTER_BLOCK, MDP_BLOCK_POWER_OFF, FALSE);
        ...
        for (i=0; i<103; i++)
        {
                if (what_clk[i] == 1)
                {
                        printk("[HSIL] %s : what clk : #%d clock is alive!!!!!!!!!!!!!!!!!!!!!!\n", __func__, i);
                }
                if (req_clk[i] == 1)
		{
                        printk("[HSIL] %s : req clk : #%d clock is alive!!!!!!!!!!!!!!!!!!!!!!\n", __func__, i);
                        if (i==42)
                        {
...
int (*set_arm11fota_mode)(void);
EXPORT_SYMBOL(set_arm11fota_mode);
#endif

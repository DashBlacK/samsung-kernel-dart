...
struct s5k5caff_work {
	struct work_struct work;
};

static struct  s5k5caff_work *s5k5caff_sensorw;
static struct  i2c_client *s5k5caff_client;

struct s5k5caff_ctrl {
...
==============================================================*/
extern struct s5k5caff_reg s5k5caff_regs;
...
int s5k5caff_regs_table_init_for_antibanding(void);
...
void sensor_DTP_control(char value);
	...
	static int s5k5caff_regs_table_write(char *name);
#endif
...
static int s5k5caff_sensor_read(unsigned short subaddr, unsigned short *data)
{
	int ret;
	unsigned char buf[2];
	struct i2c_msg msg = { s5k5caff_client->addr, 0, 2, buf };
	
	buf[0] = (subaddr >> 8);
	buf[1] = (subaddr & 0xFF);

	ret = i2c_transfer(s5k5caff_client->adapter, &msg, 1) == 1 ? 0 : -EIO;
	if (ret == -EIO) 
		goto error;
...
static int s5k5caff_sensor_write(unsigned short subaddr, unsigned short val)
{
	unsigned char buf[4];
	struct i2c_msg msg = { s5k5caff_client->addr, 0, 4, buf };

//	printk("[PGH] on write func s5k5caff_client->addr : %x\n", s5k5caff_client->addr);
//	printk("[PGH] on write func  s5k5caff_client->adapter->nr : %d\n", s5k5caff_client->adapter->nr);
	...
	return i2c_transfer(s5k5caff_client->adapter, &msg, 1) == 1 ? 0 : -EIO;
}



static int s5k5caff_sensor_write_list(struct samsung_short_t *list,int size, char *name)	
{
	int ret = 0;

#ifdef CONFIG_LOAD_FILE	
	ret = s5k5caff_regs_table_write(name);
#else
	int i = 0;

	printk("<=PCAM=> s5k5caff_sensor_write_list : %s\n", name);
...
//			printk("<=PCAM=> now SLEEP!!!!\n  %d", list[i].value);
		...
		{
		    if(s5k5caff_sensor_write(list[i].subaddr, list[i].value) < 0)
		    {
...
unsigned char s5k5caff_buf_for_burstmode[BURST_MODE_BUFFER_MAX_SIZE];
static int s5k5caff_sensor_burst_write_list(struct samsung_short_t *list,int size, char *name)	
{
	int err = -EINVAL;
	int i = 0;
	int idx = 0;
	...
	struct i2c_msg msg = {  s5k5caff_client->addr, 0, 0, s5k5caff_buf_for_burstmode};
	...
	for (i = 0; i < size; i++)
	{
		...
		switch(subaddr)
		{
			...
			case 0x0F12:
			{
				...
				if(idx ==0) 
				{
					s5k5caff_buf_for_burstmode[idx++] = 0x0F;
					s5k5caff_buf_for_burstmode[idx++] = 0x12;
				}
				s5k5caff_buf_for_burstmode[idx++] = value>> 8;
				s5k5caff_buf_for_burstmode[idx++] = value & 0xFF;
				...
				{
					msg.len = idx;
					err = i2c_transfer(s5k5caff_client->adapter, &msg, 1) == 1 ? 0 : -EIO;
			...
			{
			    idx = 0;
			    err = s5k5caff_sensor_write(subaddr, value);
			}
			break;
...
*/
	...
	switch(ctrl_info.mode)
	{
		...
		case EXT_CFG_GET_INFO:{
			...
			s5k5caff_sensor_write(0x002E, 0x23E8);
			s5k5caff_sensor_read(0x0F12, &lsb); 		//0x23E8
			s5k5caff_sensor_read(0x0F12, &msb);		//0x23EA
			s5k5caff_sensor_read(0x0F12, &rough_iso); 	//0x23EC
...
*/
		...
		case EXT_CFG_CAM_MODE_CONTROL:
		{
			...
			s5k5caff_sensor_write(0x002C, 0x0000);
			s5k5caff_sensor_write(0x002E, 0x0040);
			s5k5caff_sensor_read(0x0F12, &id);
...
void cam_pw(int status)
{
	...
	if(status == 1) //POWER ON
	{
		...
		vreg_set_level(vreg_cam_out10_vddreg, OUT1800mV);	// VDD_REG 1.8V		
		...
		pcam_msm_i2c_pwr_mgmt(s5k5caff_client->adapter, 1);
	}
	else //POWER OFF
	{
		...
		pcam_msm_i2c_pwr_mgmt(s5k5caff_client->adapter, 0); 
...
//	unsigned int	before_time, after_time, i;//I2C SPEED TEST
	...
	udelay(1);
	gpio_tlmm_config(GPIO_CFG(15, 1, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_16MA), GPIO_ENABLE);
	
	//TASS NOT USE gpio_set_value(1, 1); //STBY UP   
	gpio_set_value(0, 1); //RESET UP   
	udelay(90);			//shlee 101230 change the power sequence by hw request
...
static long s5k5caff_set_effect(int mode, int effect)
{
	long rc = 0;
	...
	s5k5caff_effect = effect;
	
	return rc;
}

static long s5k5caff_set_sensor_mode(int mode)
{
	//long rc = 0;
	...
	if(first_start_camera)
	{
		...
		S5K5CAFF_WRITE_LIST(s5k5caff_init0);
		#else
		S5K5CAFF_BURST_WRITE_LIST(s5k5caff_init0);
		#endif
		...
		#if 1 //shlee_temp #if defined(CONFIG_SAMSUNG_LTN_COMMON)
		{
			...
			rc = s5k5caff_regs_table_init_for_antibanding();	
			if (rc < 0) {
				printk("s5k5caff_sensor_init failed!\n");
			}		
		}
		#endif //CONFIG_SAMSUNG_LTN_COMMON
		//hmin84.park 101202
	...
	case SENSOR_PREVIEW_MODE:
	{
		...
		if(mMode)
		{
			...
			S5K5CAFF_WRITE_LIST(s5k5caff_camcorder_set);	
			S5K5CAFF_WRITE_LIST(s5k5caff_fps_30fix);	
		}
		else
		{
		...
		*/
		...
		{
			S5K5CAFF_WRITE_LIST(s5k5caff_preview);	
		}
	...
	case SENSOR_SNAPSHOT_MODE:
	{
		...
		s5k5caff_sensor_write(0x002C, 0x7000);
		s5k5caff_sensor_write(0x002E, 0x032E);
		s5k5caff_sensor_read(0x0F12, &lsb);
		s5k5caff_sensor_read(0x0F12, &msb);
		...
		s5k5caff_sensor_write(0x002C, 0x7000);
		s5k5caff_sensor_write(0x002E, 0x2448);
		s5k5caff_sensor_read(0x0F12, &lsb);
		s5k5caff_sensor_read(0x0F12, &msb);
		...
		if(cur_lux > base_high)
		{
			...
			S5K5CAFF_WRITE_LIST(s5k5caff_high_snapshot);
		}
		else if(cur_lux < base_low)
		{
			if((mScene == EXT_CFG_SCENE_NIGHTSHOT) ||(mScene == EXT_CFG_SCENE_FIREWORK) )
			{
				...
				S5K5CAFF_WRITE_LIST(s5k5caff_night_snapshot);	
				msleep(300);	
			}
			else
			{
				...
				S5K5CAFF_WRITE_LIST(s5k5caff_lowlight_snapshot);
				msleep(250);	
			}
		}
		else
		{
			...
			S5K5CAFF_WRITE_LIST(s5k5caff_snapshot);
		}
...
static int s5k5caff_sensor_init_probe(const struct msm_camera_sensor_info *data)
{
	int rc = 0;

	#ifndef CONFIG_LOAD_FILE
	S5K5CAFF_WRITE_LIST(s5k5caff_pre_init0);
	#endif
...
static char *s5k5caff_regs_table = NULL;

static int s5k5caff_regs_table_size;

void s5k5caff_regs_table_init(void)
{
	struct file *filp;
	char *dp;
	long l;
...
//	int i;
	...
	set_fs(get_ds());
#if 0
	filp = filp_open("/data/camera/s5k5caff.h", O_RDONLY, 0);
#else
	filp = filp_open("/mnt/sdcard/s5k5caff.h", O_RDONLY, 0);
#endif
	if (IS_ERR(filp)) {
	...
	s5k5caff_regs_table = dp;
	
	s5k5caff_regs_table_size = l;

	*((s5k5caff_regs_table + s5k5caff_regs_table_size) - 1) = '\0';

//	printk("s5k5caff_regs_table 0x%x, %ld\n", dp, l);
}

void s5k5caff_regs_table_exit(void)
{
	printk("%s %d\n", __func__, __LINE__);
	if (s5k5caff_regs_table) {
		kfree(s5k5caff_regs_table);
		s5k5caff_regs_table = NULL;
	}	
}
...
static int s5k5caff_regs_table_write(char *name)
{
	...
	*(data_buf + 6) = '\0';
	...
	start = strstr(s5k5caff_regs_table, name);
...
//			printk("addr 0x%04x, value 0x%04x\n", addr, value);
			...
			{
				if( s5k5caff_sensor_write(addr, value) < 0 )
				{
...
//latin_cam : support anti-banding
#if 1 //shlee_temp #if defined(CONFIG_SAMSUNG_LTN_COMMON)
cam_antibanding_setting s5k5caff_antibanding = CAM_ANTIBANDING_60HZ;


int s5k5caff_get_anti_banding(void)
{
	return s5k5caff_antibanding;
}

int s5k5caff_reg_init_60hz(void)
{
	printk("Init Table for Anti-banding 60 Hz\n");
	S5K5CAFF_WRITE_LIST(s5k5caff_init_60hz);
}

int s5k5caff_regs_table_init_for_antibanding(void)
{
	int rc;
	printk("s5k5caff_regs_table_init_for_antibanding() : anti-banding ==> %d Hz\n", s5k5caff_get_anti_banding());

	if (CAM_ANTIBANDING_60HZ == s5k5caff_get_anti_banding())
		rc = s5k5caff_reg_init_60hz();
	return rc;
}

ssize_t s5k5caff_antibanding_show (struct device *dev, struct device_attribute *attr, char *buf)
{
	int count;

	count = sprintf(buf, "%d", s5k5caff_antibanding);
	printk("The value of s5k5caff_antibanding is %d\n", s5k5caff_antibanding);
	return count;
}

ssize_t s5k5caff_antibanding_store (struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int tmp = 0;
	...
	if ((CAM_ANTIBANDING_50HZ == (cam_antibanding_setting) tmp)
		...
		s5k5caff_antibanding = (cam_antibanding_setting) tmp;
		printk("The s5k5caff_antibanding is set to %d\n",s5k5caff_antibanding);
	}

	return count;
}

static struct device_attribute s5k5caff_antibanding_attr = {
	.attr = {
		.name = "anti-banding",
		.mode = (S_IRUGO | S_IWUGO)},
	.show = s5k5caff_antibanding_show,
	.store = s5k5caff_antibanding_store
};
#endif //CONFIG_SAMSUNG_LTN_COMMON
//hmin84.park 101202
...
int s5k5caff_sensor_init(const struct msm_camera_sensor_info *data)
{
	...
	if (!s5k5caff_ctrl) {
		CDBG("s5k5caff_init failed!\n");
		rc = -ENOMEM;
		goto init_done;
	}

	if (data)
		s5k5caff_ctrl->sensordata = data;
...
#ifdef CONFIG_LOAD_FILE
	s5k5caff_regs_table_init();
#endif	

	rc = s5k5caff_sensor_init_probe(data);
	if (rc < 0) {
		CDBG("s5k5caff_sensor_init failed!\n");
		goto init_fail;
	}
...
static int s5k5caff_init_client(struct i2c_client *client)
{
	/* Initialize the MSM_CAMI2C Chip */
	init_waitqueue_head(&s5k5caff_wait_queue);
	return 0;
}

int s5k5caff_sensor_config(void __user *argp)
{
	...
	CDBG("s5k5caff_ioctl, cfgtype = %d, mode = %d\n",
		cfg_data.cfgtype, cfg_data.mode);

		switch (cfg_data.cfgtype) {
		case CFG_SET_MODE:
			rc = s5k5caff_set_sensor_mode(
						cfg_data.mode);
			break;

		case CFG_SET_EFFECT:
			rc = s5k5caff_set_effect(cfg_data.mode,
						cfg_data.cfg.effect);
			break;
...
int s5k5caff_sensor_release(void)
{
	int rc = 0;
	...
	printk("<=PCAM=> s5k5caff_sensor_release\n");

	kfree(s5k5caff_ctrl);
	/* up(&s5k5caff_sem); */

#ifdef CONFIG_LOAD_FILE
	s5k5caff_regs_table_exit();
#endif

	return rc;
}

static int s5k5caff_i2c_probe(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	...
	s5k5caff_sensorw =
		kzalloc(sizeof(struct s5k5caff_work), GFP_KERNEL);
	...
	i2c_set_clientdata(client, s5k5caff_sensorw);
	s5k5caff_init_client(client);
	s5k5caff_client = client;


	CDBG("s5k5caff_probe succeeded!\n");

	return 0;

probe_failure:
	kfree(s5k5caff_sensorw);
	s5k5caff_sensorw = NULL;
	CDBG("s5k5caff_probe failed!\n");
	return rc;
}

static const struct i2c_device_id s5k5caff_i2c_id[] = {
	{ "s5k5caff", 0},
	{ },
};

static struct i2c_driver s5k5caff_i2c_driver = {
	.id_table = s5k5caff_i2c_id,
	.probe  = s5k5caff_i2c_probe,
	.remove = __exit_p(s5k5caff_i2c_remove),
	.driver = {
		.name = "s5k5caff",
	},
};

static int s5k5caff_sensor_probe(const struct msm_camera_sensor_info *info,
				struct msm_sensor_ctrl *s)
{

	int rc = i2c_add_driver(&s5k5caff_i2c_driver);
	if (rc < 0 || s5k5caff_client == NULL) {
		rc = -ENOTSUPP;
		goto probe_done;
	}
	...
	rc = s5k5caff_sensor_init_probe(info);
	if (rc < 0)
		goto probe_done;
	...
	s->s_init = s5k5caff_sensor_init;
	s->s_release = s5k5caff_sensor_release;
	s->s_config  = s5k5caff_sensor_config;
...
static int __s5k5caff_probe(struct platform_device *pdev)
{
//latin_cam : support anti-banding
#if 1//shlee_temp #if defined(CONFIG_SAMSUNG_LTN_COMMON)
	int rv;
	rv = device_create_file(&pdev->dev,&s5k5caff_antibanding_attr);
	if(rv)	
		printk("<=PCAM=> s5k5caff_probe() : device_create_file() is failed with %d", rv);

#endif //CONFIG_SAMSUNG_LTN_COMMON
//hmin84.park 101202

	return msm_camera_drv_start(pdev, s5k5caff_sensor_probe);
}

static struct platform_driver msm_camera_driver = {
	.probe = __s5k5caff_probe,
	.driver = {
...
static int __init s5k5caff_init(void)
{
	return platform_driver_register(&msm_camera_driver);
}

module_init(s5k5caff_init);

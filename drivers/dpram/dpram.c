...
static ssize_t show_tftest(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	char *sd_card_str = sdcard_slot_status ? "Insert" :"Remove";

	return snprintf(buf, PAGE_SIZE, "%s\n", sd_card_str);
}


static DEVICE_ATTR(tftest, S_IRUGO, show_tftest, NULL);
...
//	SmemBase = (volatile unsigned char *)(smem_alloc(SMEM_ID_VENDOR0, 0x4000*2));
	SmemBase = (volatile unsigned char *)(smem_alloc(SMEM_ID_VENDOR0, DPRAM_SIZE)); //0x10000
...
static int __init dpram_init(void)
{
	...
	tctest_dev = device_create(sec_class, NULL, 0, NULL, "tctest");
	if(IS_ERR(tctest_dev))
		pr_err("Failed to create device(tctest)!\n");

	if(device_create_file(tctest_dev, &dev_attr_tftest) < 0)

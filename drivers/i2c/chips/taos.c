...
/* [HSS] This value depends on each model's H/W. It needs tunning */
...
#elif defined(CONFIG_MACH_COOPER) ||defined(CONFIG_MACH_GT2) || defined(CONFIG_MACH_GIOATT)
	#define PRX_THRSH_HI_PARAM		650
	#define PRX_THRSH_LO_PARAM		550	
	...
	#define PRX_THRSH_HI_PARAM		600
	#define PRX_THRSH_LO_PARAM		450
#elif defined(FEATURE_SAMSUNG_FLIPBOOK)
	#define PRX_THRSH_HI_PARAM		600
	#define PRX_THRSH_LO_PARAM		550
#else			
...
#if defined(CONFIG_MACH_BENI) || defined(CONFIG_MACH_COOPER) || defined(CONFIG_MACH_GIOATT)
#define PRX_PULSE_CNT_PARAM		0x0A //0x0F
#elif defined(CONFIG_MACH_TASS) || defined(CONFIG_MACH_TASSATT) || defined(CONFIG_MACH_TASSTMO)|| defined(CONFIG_MACH_TASSMCT) || defined(CONFIG_MACH_TASSVTR) || defined(CONFIG_MACH_GT2)
...
#elif defined(CONFIG_MACH_BENI) || defined(CONFIG_MACH_FLIPBOOK) 
...
/* Basic Software Operation for TMD2771X Module */
	...
	/* gpio config */
	config = GPIO_CFG(GPIO_SENSE_OUT, 0, GPIO_INPUT, GPIO_NO_PULL, GPIO_2MA);
...
static int taos_opt_resume(struct i2c_client *client)
{
	...
	if(proximity_enable)
	{
		...
		{
			taos_irq_handler(taos->irq,taos);

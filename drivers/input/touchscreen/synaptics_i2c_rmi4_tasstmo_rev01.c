...
// CAUTION: commented out for SMD test
...
#define HEX_SW_VER_REV04    0x08 

#define MAX_X	240//320 
#define MAX_Y	320//480 

#define TOUCH_VERIFY_WITH_TIMER
...
int key_led_power_control(uint8_t LED_ON_OFF)
{
	...
	DPRINTK(KERN_ERR "%s: key_led_power_control ON/OFF [%d]\n",__func__, LED_ON_OFF);
...
static void synaptics_ts_work_func(struct work_struct *work)
{
	...
	if (tsp_log_enable>=1)	
		printk("[TSP] touch_key_new[%d],touch_key_old[%d]\n",touch_key_new,touch_key_old);
...
static void check_ic_work_func(struct work_struct *work)
{
	...
	*/
	if (tsp_log_enable>=2)
		printk("[TAOS] check_ic_..: pre_ta_stat = %d, tsp_charger_t.. = %d\n", pre_ta_stat, tsp_charger_type_status );
		...
		struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(board_hw_revision >= 0x3) {
		if (tsp_log_enable>=1)
			printk("[TSP] board_hw_revision %d\n", board_hw_revision);
	...
	if(((HW_ver <= HEX_HW_VER ) || ( HW_ver < HEX_HW_VER && HW_ver > 0 )) && (board_hw_revision < 0x3)) {
		if((SW_ver<HEX_SW_VER)||((SW_ver&0xF0)==0xF0)||(SW_ver==0)){
			DPRINTK(KERN_INFO "[TSP] Firmware Update on REV01");
			firm_update();
		} else {
			DPRINTK(KERN_INFO "[TSP] Firmware version REV01 is up-to-date");
		}	
		...
		if((SW_ver < HEX_SW_VER_REV03)||((SW_ver&0xF0)==0xF0)||(SW_ver==0)){
			DPRINTK(KERN_INFO "[TSP] Firmware Update on REV03");
			firm_update();
		}else{
			DPRINTK(KERN_INFO "[TSP] Firmware version REV03 is up-to-date");
		}
		...
		if((SW_ver < HEX_SW_VER_REV04)||((SW_ver&0xF0)==0xF0)||(SW_ver==0)){
			DPRINTK(KERN_INFO "[TSP] Firmware Update on REV04");
			firm_update();
		}else{
			DPRINTK(KERN_INFO "[TSP] Firmware version REV04 is up-to-date");
		}
	}
...
int set_tsp_for_ta_detect(int state)
{
 ...
 DPRINTK("[TSP] ++++set_tsp_for_ta_detect\n");
 ...
 if(state)
 {
	DPRINTK("[TSP] [1] set_tsp_for_ta_detect!!! state=1\n");
 ...
 {
	DPRINTK("[TSP] [2] set_tsp_for_ta_detect!!! state=0\n");
	wdog_val[0] = wdog_val[0] & ~(1 << 7);
	ret = tsp_i2c_write( i2c_addr, wdog_val, sizeof(wdog_val));
	pre_ta_stat = 0;
 }
...
int set_tsp_for_prox_enable(int state)
{
 ...
 if(state)
 {
	printk("[TSP] [1] set_tsp_for_prox_enable!!! state=1\n");
 	wdog_val[0] = wdog_val[0] | (1 << 6);
 	ret = tsp_i2c_write( i2c_addr, wdog_val, sizeof(wdog_val));
 	pre_prox_stat = 1;
 }
 else
 {
	printk("[TSP] [2] set_tsp_for_prox_enable!!! state=0\n");
 	wdog_val[0] = wdog_val[0] & ~(1 << 6);
 	ret = tsp_i2c_write( i2c_addr, wdog_val, sizeof(wdog_val));
 	pre_prox_stat = 0;
 }
...
static ssize_t firmware_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	HW_ver = buf_show[0];
	SW_ver = buf_show[1];
	DPRINTK("[TSP] %s:%d, ver SW=%x, HW=%x\n", __func__,__LINE__, SW_ver, HW_ver );

	if ( HW_ver >= 0x01 ) {
		if(board_hw_revision == 0x3) {
			phone_ver = HEX_SW_VER_REV03*10;
		} else if(board_hw_revision >= 0x4) {
			phone_ver = HEX_SW_VER_REV04*10;
		} else {
		phone_ver = HEX_SW_VER*10;
	}
	} else {
		if (tsp_log_enable>=1)
			printk("[TSP] %s:%d, HW_ver=%d is wrong!!\n", __func__,__LINE__,HW_ver);
	}
...
void emergency_firm_update( void )
{
	...
	do {
		DPRINTK("[TSP]-------------------------------!!\n");
		DPRINTK("[TSP] Emergency TSP Firmware Update Start ~~~!!\n");
		tsp_reset();
		ret_val = firm_update();
		msleep(200);
		if (tsp_log_enable>=1)
...
int firm_update( void )
{
	...
	// TEST
	DPRINTK("[TSP] TMO %s:%d, ver SW=%x, HW=%x\n", __func__,__LINE__, SW_ver, HW_ver );

	gpio_configure( TSP_SCL, GPIOF_DRIVE_OUTPUT );

	if(board_hw_revision == 0x3) {
		DPRINTK(KERN_INFO "[TSP] %s, %d cypress_update blocked, HW ver=%d\n", __func__, __LINE__, HW_ver);

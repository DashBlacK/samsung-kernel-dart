...
#if defined(CONFIG_MACH_COOPER) || defined(CONFIG_MACH_BENI) || defined(CONFIG_MACH_TASS)|| defined(CONFIG_MACH_LUCAS) || defined(CONFIG_MACH_TASSATT) || defined(CONFIG_MACH_TASSTMO)|| defined(CONFIG_MACH_TASSMCT) || defined(CONFIG_MACH_TASSVTR)  || defined(CONFIG_MACH_GT2) || defined(CONFIG_MACH_FLIPBOOK) || defined(CONFIG_MACH_GIOATT)
unsigned int Volume_Up_irq = 0;
unsigned int Volume_Down_irq = 0;
#endif

extern int board_hw_revision;
#if defined(CONFIG_MACH_COOPER) || defined(CONFIG_MACH_BENI) || defined(CONFIG_MACH_TASS)  || defined(CONFIG_MACH_TASSATT) || defined(CONFIG_MACH_TASSTMO) || defined(CONFIG_MACH_TASSMCT) || defined(CONFIG_MACH_TASSVTR)  || defined(CONFIG_MACH_GT2) || defined(CONFIG_MACH_FLIPBOOK) || defined(CONFIG_MACH_GIOATT)
extern void TSP_forced_release_forkey(void);
#endif


int key_log_enable=0;
...
#if defined (CONFIG_MACH_TASSATT) 
#define GPIO_KBC0 33
#elif defined(CONFIG_MACH_TASSTMO) || defined(CONFIG_MACH_TASSMCT) || defined(CONFIG_MACH_TASSVTR) 
#define GPIO_KBC0 35
#elif defined(CONFIG_MACH_GT2)
#define GPIO_KBC0 35
static unsigned int GPIO_SLIDE = 124;	// temp
#elif defined(CONFIG_MACH_FLIPBOOK) //ADD_FB key
#define GPIO_KBC0 35
static unsigned int GPIO_SLIDE = 94;
#endif
...
#if defined(CONFIG_MACH_COOPER) || defined(CONFIG_MACH_GT2) ||defined(CONFIG_MACH_BENI) || defined(CONFIG_MACH_TASS) || defined(CONFIG_MACH_TASSTMO)|| defined(CONFIG_MACH_TASSMCT) || defined(CONFIG_MACH_TASSVTR) || defined(CONFIG_MACH_TASSATT)|| defined(CONFIG_MACH_FLIPBOOK) || defined(CONFIG_MACH_GIOATT)
		...
		if (keycode == KEY_RESERVED) {
			...
			if (mi->flags & GPIOKPF_PRINT_MAPPED_KEYS)
				pr_info("gpiomatrix: key %x, %d-%d (%d-%d) "
					...
					out, in, mi->output_gpios[out],
					mi->input_gpios[in], pressed);
#if defined(CONFIG_MACH_EUROPA) || defined(CONFIG_MACH_CALLISTO)  || defined(CONFIG_MACH_COOPER) || defined(CONFIG_MACH_BENI) || defined(CONFIG_MACH_TASS)|| defined(CONFIG_MACH_TASSTMO)|| defined(CONFIG_MACH_TASSMCT) || defined(CONFIG_MACH_TASSVTR) || defined(CONFIG_MACH_TASSATT)|| defined(CONFIG_MACH_FLIPBOOK) || defined(CONFIG_MACH_LUCAS)  || defined(CONFIG_MACH_GIOATT)
			if(!power_off_done) {
#if defined(CONFIG_MACH_LUCAS)
                                ...
                                else if ((alt_key_pressed) && (keycode == KEY_UP))
...
#if defined(CONFIG_MACH_COOPER) || defined(CONFIG_MACH_BENI) || defined(CONFIG_MACH_TASS)|| defined(CONFIG_MACH_TASSTMO)|| defined(CONFIG_MACH_TASSMCT) || defined(CONFIG_MACH_TASSVTR) || defined(CONFIG_MACH_TASSATT)|| defined(CONFIG_MACH_FLIPBOOK)
				if(keycode == KEY_HOME && pressed == 1)
					TSP_forced_release_forkey();
#endif
#if defined(CONFIG_MACH_CALLISTO)  || defined(CONFIG_MACH_COOPER) || defined(CONFIG_MACH_BENI) || defined(CONFIG_MACH_TASS)|| defined(CONFIG_MACH_TASSTMO)|| defined(CONFIG_MACH_TASSMCT) || defined(CONFIG_MACH_TASSVTR) || defined(CONFIG_MACH_TASSATT)|| defined(CONFIG_MACH_GT2)|| defined(CONFIG_MACH_FLIPBOOK) || defined(CONFIG_MACH_LUCAS)
                        if (key_log_enable>=1)
				printk("key event (keycode:%d, pressed:%d), w=%d\n", 
						keycode, pressed, lcd_on_state_for_debug);	// sec: sm.kim
	        	      else
        		        printk("key event (pressed:%d)\n",pressed);
				// hsil	
//				if (keycode == 115)
//				{
//					volup_cnt++;
...
static irqreturn_t gpiokey_irq_handler(int irq_in, void *dev_id)
{
	...
	else if (key_status == 1) // 1 : release
	{
		...
		printk("key event (keycode:%d, pressed:%d)\n", KEY_END, 0); //sec: sm.kim
        		        ...
        		        printk("key event (pressed:%d)\n",0);
		key_pressed = 0;
#if defined(CONFIG_MACH_COOPER)|| defined(CONFIG_MACH_TASSTMO)|| defined(CONFIG_MACH_TASSMCT) || defined(CONFIG_MACH_TASSVTR) || defined(CONFIG_MACH_TASSATT)|| defined(CONFIG_MACH_FLIPBOOK) 
		TSP_forced_release_forkey();
#endif
	}
...
#if defined(CONFIG_MACH_CALLISTO) || defined (CONFIG_MACH_GT2) || defined(CONFIG_MACH_FLIPBOOK) //ADD_FB key
// hsil
#ifdef SLIDE_DEBOUNCE_CHECK
static struct timer_list slide_deb_tmr;
static int slide_deb_chk = -1;

void slide_debounce_check(unsigned long arg )
{
	struct gpio_kp *kp = (struct gpio_kp *)arg;
	struct gpio_event_matrix_info *mi = kp->keypad_info;
	unsigned short keyentry = mi->keymap[0];
	unsigned short dev = keyentry >> MATRIX_CODE_BITS;
	del_timer(&slide_deb_tmr);
	if((gpio_get_value(GPIO_SLIDE) ^ 1) == slide_deb_chk)
	{
		input_report_switch(kp->input_devs->dev[dev], SW_LID, slide_deb_chk);
		input_sync(kp->input_devs->dev[dev]);
		printk("[SLIDE] send Slide state (%d)\n", slide_deb_chk);
	}
			 
	enable_irq(gpio_to_irq(GPIO_SLIDE));

}
static irqreturn_t slide_int_handler(int irq, void *dev_id)
{
	struct gpio_kp *kp = dev_id;
	int state = (gpio_get_value(GPIO_SLIDE) ^ 1);
	if (state == slide_deb_chk)
		return IRQ_HANDLED;
	disable_irq_nosync(gpio_to_irq(GPIO_SLIDE));
	slide_deb_chk = state;
//	printk("[SLIDE] changed Slide state (%d)\n", slide_deb_chk);
	init_timer( &slide_deb_tmr );
	slide_deb_tmr.expires = get_jiffies_64() + HZ/3;
	slide_deb_tmr.data = (unsigned long)kp;
	slide_deb_tmr.function = slide_debounce_check;
	add_timer( &slide_deb_tmr );
...
#if defined(CONFIG_MACH_COOPER) || defined(CONFIG_MACH_BENI) || defined(CONFIG_MACH_TASS) || defined(CONFIG_MACH_TASSATT) || defined(CONFIG_MACH_TASSTMO)|| defined(CONFIG_MACH_TASSMCT) || defined(CONFIG_MACH_TASSVTR) 
...
#elif defined (CONFIG_MACH_GT2) || defined(CONFIG_MACH_FLIPBOOK) //ADD_FB key
			if((mi->input_gpios[i]) == 76)	// volup, voldown kbr
				Volume_Up_irq = irq;
#endif		
#if defined(CONFIG_MACH_LUCAS)
		switch(mi->input_gpios[i])
...
#if !defined (CONFIG_MACH_LUCAS) && !defined(CONFIG_MACH_GT2)&& !defined(CONFIG_MACH_TASSATT)&& !defined(CONFIG_MACH_TASSTMO)&& !defined(CONFIG_MACH_TASSMCT) && !defined(CONFIG_MACH_TASSVTR)  && !defined(CONFIG_MACH_FLIPBOOK) //ADD_FB key
...
//#if defined (CONFIG_MACH_TASS) || defined (CONFIG_MACH_GT2) || defined(CONFIG_MACH_TASSATT) || defined(CONFIG_MACH_TASSTMO) || defined(CONFIG_MACH_TASSMCT) || defined(CONFIG_MACH_TASSVTR)  || defined(CONFIG_MACH_FLIPBOOK) 

//	err = set_irq_wake(gpio_to_irq(GPIO_KBR0), 1);
//	if (err) 
//	{
...
#ifdef CONFIG_MACH_GT2
//mootest2
	err = set_irq_wake(gpio_to_irq(GPIO_KBR5), 1);
	if (err) 
	{
		pr_err("gpiomatrix: set_irq_wake failed for input %d, "	"irq %d\n", mi->input_gpios[i], irq);
	}
#endif
#if defined(CONFIG_MACH_CALLISTO) || (CONFIG_MACH_GT2) || defined(CONFIG_MACH_FLIPBOOK) //ADD_FB key
	// hsil
	...
	gpio_direction_input(GPIO_SLIDE);
...
#if defined(CONFIG_MACH_COOPER) || defined(CONFIG_MACH_BENI) || defined(CONFIG_MACH_TASS)|| defined(CONFIG_MACH_TASSTMO) || defined(CONFIG_MACH_TASSMCT) || defined(CONFIG_MACH_TASSVTR) || defined(CONFIG_MACH_TASSATT)|| defined(CONFIG_MACH_GT2)|| defined(CONFIG_MACH_FLIPBOOK)
#if defined(CONFIG_KERNEL_SEC_MMICHECK)
